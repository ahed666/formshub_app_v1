...
//   if message edit success
    public function messageeditsuccess($id)
    {
        ...
        $this->current_form = Form::join('logos', 'logos.id', '=', 'forms.logo_id')
            ->where('forms.account_id', Auth::user()->current_account_id)->where('forms.id', $this->current_form_id)->select('forms.id as form_id', 'forms.*', 'logos.logo_url')->get()[0];
        if ($this->current_form != null) {
            $this->current_form_id = $this->current_form->form_id;
        } else {
            $this->current_form = Form::where('forms.account_id', Auth::user()->current_account_id)->where('forms.id', $this->current_form_id)->get()[0];
    ...
    {
        $this->current_form=Form::join('logos', 'logos.id', '=', 'forms.logo_id')->where('forms.account_id', Auth::user()->current_account_id)->where('forms.id', $this->current_form_id)->select('forms.id as form_id', 'forms.*', 'logos.logo_url')->get()[0];
    ...
    public function mount($id,$lastLocal)
    {
        ...
        $this->current_form = Form::join('logos', 'logos.id', '=', 'forms.logo_id')->where('forms.account_id', Auth::user()->current_account_id)->orderBy('forms.created_at', 'desc')
         ->where('forms.id','=',$this->current_form_id)->select('forms.id as form_id', 'forms.*', 'logos.logo_url')->get()[0];
