...
<div   wire:loading.class="disabled opacity-50" class=" p-4 xs:p-1">
    ...
    <div class="flex justify-between items-center   mb-1">
        <div class="flex space-x-4 items-center">
           ...
           <button {{ $questionIndex>0?"":"disabled" }} wire:click="backQuestion" class="focus:outline-none"><!DOCTYPE svg PUBLIC "-//W3C//DTD SVG 1.1//EN" "http://www.w3.org/Graphics/SVG/1.1/DTD/svg11.dtd">
            ...
            <div class="flex items-center justify-center space-x-0 w-72 xs:w-40 overflow-x-auto">
                ...
                @for($i = 0; $i < count($formquestions); $i++)
                     <button wire:click="currentQuestion({{ $i }})" class="{{$questionIndex==$i?"bg-secondary_blue":""  }} focus:outline-none border-[1px]  border-gray-200 p-1">{{ $i+1 }}</button>
                @endfor
           ...
           <button {{ $questionIndex<count($formquestions)-1?"":"disabled" }} wire:click="nextQuestion"  class="focus:outline-none"><!DOCTYPE svg PUBLIC "-//W3C//DTD SVG 1.1//EN" "http://www.w3.org/Graphics/SVG/1.1/DTD/svg11.dtd">
        ...
        {{-- @foreach($formquestions as $key => $question) --}}
            <div id="question_template"  class="grid grid-cols-12  bg-white my-1 rounded-[0.5rem] border-[1px] border-gray-300  p-3 ">
                ...
                <div class="flex justify-between items-center xs:block md:block col-span-12 md:row-span-1 xs:row-span-1 my-1 ">
                    <div class="grid w-[60%] xs:w-full mr-1 mb-2 mt-2">
                        <div class="flex justify-start items-center ">
                            <h1 class="text-secondary_blue">{{ __('main.question') }}</h1>
                             {{-- question show status --}}
                            <div id="question_show" class="mx-4 justify-center items-center">
                        ...
                        <div class="bg-gray-50 px-2 flex  justify-center items-center col-span-12 md:row-span-1 xs:row-span-1 min-h-[70px] max-h-[70px]">
                            <h1 id="question_text" class="text-black  xs:text-xs md:text-sm font-bold text-md">

                            </h1>
                        </div>
                    </div>
                    <div class=" flex items-center justify-between xs:block md:block md:row-span-1 xs:row-span-1 w-[40%] md:w-full lg:w-full  xs:w-full ml-1 mb-2 mt-2 ">
                        {{-- question info --}}
                        <div class="grid xs:flex">
                            ...
                            <div class="w-full flex  items-center my-[2px] ">
                                ...
                                <div class="mx-1"><h1 class="text-sm xs:text-xs md:text-sm"><span id="age-{{ $currentQuestion->id }}" class=""></span></h1></div>
                            </div>
                            {{-- answers --}}
                            <div class="w-full flex  items-center my-[2px]">
                                <div class="mx-1">
                                ...
                                <div class="mx-1"> <h1 class="text-sm xs:text-xs md:text-sm"><span id="total-answers-{{ $currentQuestion->id }}" class=""></span>{{ __('main.answers') }}</h1></div>
                            </div>
                            {{-- skip --}}
                            <div class="w-full flex  items-center my-[2px]">
                                <div class="mx-1">
                                ...
                                <div class="mx-1"><h1 class="text-sm xs:text-xs md:text-sm"><span id="total-skipped-{{ $currentQuestion->id }}" class=""></span>{{ __('main.skips') }}</h1></div>
                            </div>
                        </div>
    ...
    var QuestionText=document.getElementById('question_text');
     ...
     var numofquestions=0;
        ...
        var satisfactionRates={
                 "Very Satisfied":4,"راضي تماماٌ":4,"Ganap na Nasiyahan":4,"مکمل طور پر مطمئن":4,
                 "Satisfied":3,"راضي":3,"Nasiyahan":3,"مطمئن":3,
                 "Natural":2,"محايد":2,"Natural":2,"غیر جانبدار":2,
                 "Unsatisfied":1,"غير راضي":1,"غیر مطمئن":1,"Hindi Nasisiyahan":1,
                 "Very Unsatisfied":0,"غير راضي تماماٌ":0,"مکمل طور پر غیر مطمئن":0,"Ganap na Hindi Nasisiyahan":0,
          };
        var paths={
        ...
        var responseColumnsTranslations={
            "ar":{"responseid":"الرقم","responsedate":"التاريخ","answer":"الأجابة"},
            "en":{"responseid":"Id","responsedate":"Date","answer":"Answer"},
            "ur":{"responseid":"آئی ڈی","responsedate":"تاریخ","answer":"جواب دیں۔"},
            "tl":{"responseid":"Id","responsedate":"petsa","answer":"Sagot"}
        }
        ...
        function Createexcelfile(data,question,start,end,type,Typeunstatisitcs,filePath,single=false,questionIndex)
        {
                ...
                if(type=="statistics"){
                    ...
                    data.question_data.forEach(function(answer,i) {
                        ...
                        if(answer.answer_sum==max&&max>0){
                            ...
                            sheet.cell(`F${i+15}`).style({ fill: { type: 'solid', color: '90EE90' } });
                        }
            ...
            const blob = new Blob([fileData],{details:question.question_details},{ type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
        ...
        function  CreateZip(excelFiles)
        {
            ...
            zip.file(fileName, excelFile, { binary: true });
            }
        ...
        {
            for(var a=[],d=new Date(s);d<=new Date(e);d.setDate(d.getDate()+1)){ a.push(new Date(d));}return a;
        };
