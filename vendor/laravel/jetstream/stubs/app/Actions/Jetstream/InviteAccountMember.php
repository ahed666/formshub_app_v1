...
class InviteAccountMember implements InvitesAccountMembers
{
    ...
    protected function rules(Account $account): array
    {
        return array_filter([
            'email' => [
                'required', 'email',
                Rule::unique('team_invitations')->where(function (Builder $query) use ($account) {
                    $query->where('team_id', $account->id);
                }),
            ],
     ...
     */
    protected function ensureUserIsNotAlreadyOnTeam(Account $account, string $email): Closure
    {
